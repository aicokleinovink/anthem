...
test.describe('expanding', () => {
  test('grows into the card slot and back, from the button', async ({ page, control }) => {
    await open(page);
    await control.player(TRACK);
    ...
    await expect(player(page).getByRole('button', { name: 'Expand player' })).toHaveCount(0);
  ...
  test('a section tab both closes it and navigates', async ({ page, control }) => {
    await open(page);
    await control.player(TRACK);
    await player(page).getByRole('button', { name: 'Expand player' }).click();
    await expect(player(page).getByRole('button', { name: 'Collapse player' })).toBeVisible();
  ...
  test('a dragged volume slider does not flood the receiver', async ({ page, control }) => {
    await open(page);
    await control.player(TRACK);
    await player(page).getByRole('button', { name: 'Expand player' }).click();
    ...
    await expect
      ...
      .toMatchObject({ '1': -55 });
    const writes = (await control.wire()).filter((frame) => frame.startsWith('Z1PVOL'));
